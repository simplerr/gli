...
namespace gli
{
	namespace detail
	{
		...
		inline glm::vec4 decompress_dxt3(const dxt3_block &Block, const extent2d &BlockTexelCoord)
		{
			...
			glm::uint8_t ColorIndex = (Block.Row[BlockTexelCoord.y] >> (BlockTexelCoord.x * 2)) & 0x3;
		...
		inline glm::vec4 decompress_dxt5(const dxt5_block &Block, const extent2d &BlockTexelCoord)
		{
			...
			Bitmap |= glm::uint64_t(Block.AlphaBitmap[3] | (Block.AlphaBitmap[4] << 8) | (Block.AlphaBitmap[5] << 16)) << 24;

			glm::uint8_t AlphaIndex = (Bitmap >> ((BlockTexelCoord.y * 4 + BlockTexelCoord.x) * 3)) & 0x7;
		...
		inline texel_block4x4 decompress_dxt5_block(const dxt5_block &Block)
		{
			...
			for(glm::uint8_t Row = 0; Row < 4; ++Row)
			{
				for(glm::uint8_t Col = 0; Col < 4; ++Col)
				{
					glm::uint8_t ColorIndex = (Block.Row[Row] >> (Col * 2)) & 0x3;
					glm::uint8_t AlphaIndex = (Bitmap >> ((Row * 4 + Col) * 3)) & 0x7;
